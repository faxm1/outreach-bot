...
async def generate_email(recipient_email: str, company_name: str = None) -> dict:
    """Returns {"subject": str, "body": str}. Falls back to template on any failure."""
    domain = _extract_domain(recipient_email)
    if company_name:
        company_line = (
            f"The recipient organization is: {company_name}. "
            f"Use the company name naturally in the greeting or introduction "
            f"(for example: 'Dear Hiring Team at {company_name},' or a natural reference in the opening sentence). "
            f"Do not force it awkwardly."
        )
    else:
        company_line = "Do not mention any specific company name."
    user_prompt = (
...
async def generate_email_from_posting(
    ...
    if company_name:
        company_line = (
            f"Company/Organization: {company_name}\n"
            f"Use the company name naturally in the greeting or introduction. Do not force it awkwardly.\n\n"
        )
    else:
        company_line = ""
